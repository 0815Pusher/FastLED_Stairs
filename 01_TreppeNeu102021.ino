//Stairs 10_2021 2x_PIR 1x_LDR_BH1750 1x_Arduino_Mega
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#include <BH1750.h>
#include <Wire.h>
...
long LDRThreshold = 10;    // Only switch on LED's at night when LDR senses low light conditions - you may have to change this value for your circumstances! Hell=400 Dunkel= <10
...
void setup()
{
    ...
    FastLED.setMaxPowerInVoltsAndMilliamps(5, 10000); // Volt/Ampere
    FastLED.clear();
    FastLED.show();
...
void loop()
{
    ...
    if (lux > LDRThreshold)
    {
        ...
        FastLED.show();
        FastLED.clear();
        delay(30000); // wann LDR abfragen / Intervall
    }
    else
    {
    ...
    {   // the 2nd term allows timeOut to be contantly reset if one lingers at the top of the stairs before decending but will not allow the bottom PIR to reset timeOut as you decend past it.
    ...
    {   // the 2nd term allows timeOut to be contantly reset if one lingers at the bottom of the stairs before decending but will not allow the top PIR to reset timeOut as you decend past it.
        timeOut = millis(); // Timestamp when the PIR is triggered.  The LED cycle wil then start.
        downUp = 2;
        topdown(random8(0, 6)); // lights up the strip from top down
    }
...
void topdown(int mode)
{
   ...
   for (int i = 0; i < 3; i++)
   { // Helpful debug indication flashes led on Arduino board twice
